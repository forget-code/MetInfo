...
class product_database extends news_database {
  ...
  public function table_para(){
    ...
    /*public function del_plist($id = '',$module = ''){
        global $_M;
        $lang = $_M['lang'];
        if ($id && is_numeric($id) && $module && is_numeric($module)) {
            $query="delete from {$_M['table']['plist']} where listid='$id' and lang='{$lang}' and module='$module'";
            DB::query($query);
            return true;
        }
        return false;
