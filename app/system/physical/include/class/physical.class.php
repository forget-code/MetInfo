...
function filescan($jkdir,$fileback){
	...
	unset($fileback["$adminfile/app/physical/dlappfile.php"]);
	$filenamearray=array();
	$column=DB::get_all("select * from {$_M[table][column]} where classtype=1 or releclass!=0");
	...
	foreach($fileback as $key=>$val){
		if(!file_exists(PATH_WEB.$key)){
			$physical_file .="1|$key|,";
		}
		else{
			if($fileback[$key]['filesize']!=$this->met_filesize(PATH_WEB.$key)){
				$physical_file .="2|$key|,";
			}
		}
	}
	foreach($filediff as $key=>$val){
		if(!file_exists(PATH_WEB.$key)){
			...
			if($filediff[$key]['filesize']!=$this->met_filesize(PATH_WEB.$key)){
