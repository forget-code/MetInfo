        ...
        case 'next2':
            if($abt_type == 1){
                ...
                if($admin_list && $admin_list['admin_email']==''){
                    okinfo("{$_M['url']['own_form']}a=doindex",$_M['word']['password14']);
                }
                if(!$admin_list){
                    if(!is_email($admin_mobile)){
                        okinfo("{$_M['url']['own_form']}a=doindex",$_M['word']['password7']);
                    }
                    $admin_list = DB::get_one("SELECT * FROM {$_M['table']['admin_table']} WHERE admin_email='$admin_mobile' and usertype='3'");
                    if(!$admin_list){
                        okinfo("{$_M['url']['own_form']}a=doindex&langset={$_M['form']['langset']}",$_M['word']['password14']);
                    }
                }
                if($admin_list){
                    ...
                    if($_M[config][met_agents_type]<=1){
                        $body .="<div class='logo'><a href='{$_M[config][met_weburl]}' title='{$_M[config][met_webname]}'><img src='https://www.mituo.cn/upload/201812/1546050657.png' /></a></div>";
                    }
