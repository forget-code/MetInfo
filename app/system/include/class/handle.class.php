...
class handle
{
   ...
   */
    ...
    public function redirectUrl($data)
    {
        ...
        if (strstr(REQUEST_URI, '.php') && !defined('IN_ADMIN') && !$_M['form']['pageset'] && !$_M['form']['search'] && ($_M['config']['met_pseudo'] || $_M['config']['met_webhtm']) && !$_M['form']['metinfonow'] && !isset($_M['form']['para'])) {
    ...
    public function checkFunction($site_url = '')
    {
        ...
        $items = array(
            array('mysqli_connect', 'danger', '支持', '无法连接数据库',  '函数'),
            array('zip', 'danger', '支持', '无法在线解压ZIP文件。（无法通过后台上传模板和数据备份文件）',  '模块'),
            array('curl', 'danger', '支持', '系统无法远程获取内容，会导致有些操作不起作用或数据不显示',  '模块'),
            array('file_get_contents', 'danger', '支持', '系统无法远程获取内容，会导致有些操作不起作用或数据不显示',  '函数'),
            array('file_put_contents', 'danger', '支持', '系统无法写文件',  '函数'),
            array('file_uploads', 'danger', '支持', '无法上传文件',  '配置'),
            array('parse_ini_file', 'danger', '支持', '无法连接数据库',  '函数'),
            array('fopen', 'danger', '支持', '系统无法打开操作文件',  '函数'),
            array('mb_strlen', 'danger', '支持', '会导致截取内容长度时出错',  '函数'),
            array('gd', 'danger', '支持', '图片打水印和缩略生成功能无法使用',  '模块'),
            array('copy', 'danger', '支持', '无法上传或复制文件',  '函数'),
            array('smtp', 'warning', '支持', '系统邮件功能无法使用',  'smtp'),
            array('rename', 'danger', '支持', '无法重命名文件',  '函数'),
            array('unlink', 'danger', '支持', '无法清除可视化缓存',  '函数'),
            array('opendir', 'danger', '支持', '无法列出目录下文件',  '函数'),
            array('scandir', 'danger', '支持', '无法列出目录下文件',  '函数'),
            array('woff2', 'warning', '支持', '无法进行可视化编辑',  'woff'),
            array('PHP', 'danger', PHP_VERSION, 'php版本过低，请使用php5.3或以上版本', 'php'),
            array('openssl', 'warning', OPENSSL_VERSION_TEXT, OPENSSL_VERSION_TEXT.'版本建议在1.0.1以上，低于此版本可能无法使用商城支付功能', 'openssl'),
        );
        ...
        foreach ($items as &$v) {
            switch ($v[4]) {
                ...
                case 'smtp':
                    ...
                    break;
                case 'woff':
                $yes = @file_get_contents($site_url.'/app/static/fonts/font-awesome/fontawesome-webfont.woff2');
                break;
                case '类':
