...
?>
<!DOCTYPE HTML>
<html class="{$html_class} met-web oxh" {$html_hidden}>
<head>
<meta charset="utf-8">
<?php if($nofollow){ ?>
...
<meta name="generator" content="MetInfo {$c.metcms_v}" data-variable="{$url.site}|{$data.lang}|{$data.synchronous}|{$c.met_skin_user}|{$data.module}|{$data.classnow}|{$data.id}" data-user_name="{$user_name}">
...
<script>(function(){var t=navigator.userAgent;(t.indexOf("rv:11")>=0||t.indexOf("MSIE 10")>=0)&&document.write("<script src=\"{$_M["url"]["site"]}app/system/include/static/vendor/html5shiv.min.js\"><\/script>")})();</script>
<!--[if lte IE 9]>
...
<?php
if(!$_M[config][disable_cssjs]){?>
<script src="{$url.site}public/ui/v2/static/js/basic{$basic_js_name}.js?{$basic_js_time}" data-js_url="{$url.site}templates/{$c.met_skin_user}/cache/{$metpagejs}" id="met-page-js"></script>
<?php
}
?>
<?php
...
?>
<script src="{$url.site}cache/lang_json_{$data.lang}.js?{$met_lang_time}"></script>
<?php
    if($c["shopv2_open"]){
