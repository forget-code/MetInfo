...
(function() {
	...
	M.component.modal_options['.img-library-modal'] = {
		...
		modalHeaderappend: '<button type="button" class="btn btn-warning float-right btn-img-folder-back"><i class="fa-reply mr-2"></i>'+METLANG.back_folder_list+'</button>',
		modalSize: 'xl',
		modalFullheight: 1,
		modalBody: '<ul class="img-folder-list list-unstyled mb-0 row"></ul><ul class="img-library-breadcrumb list-unstyled"></ul><ul class="img-library-list list-unstyled mb-0 row mx-0"></ul><div class="d-flex align-items-center justify-content-center hide nodata" style="height:calc(100% - 38px);"><div class="h5 mb-0">' + (METLANG.nopicture || '暂无图片') + '</div></div>' + M.component.loader({
	...
	$.fn.extend({
		...
		metFileInput: function() {
			...
			var messageFun = function(obj, type, msg) {
					...
					$form_group.find('span.form-control-label').text(msg);
				},
				errorFun = function(obj, data) {
					var $obj2 = obj.parents('.file-input'),
						order = typeof data.response.order != 'undefined' ? data.response.order : data.index;
					$obj2.find('.file-preview-thumbnails .file-preview-frame .file-preview-view[data-order="' + order + '"]:not([data-size])').parents('.file-preview-frame').remove();
					// 显示报错文字
					messageFun($obj2, 'error', data.response.error);
				},
				successFun = function(obj1, data, multiple) {
					var $obj1 = obj1.parents('.input-group').find('input[type="text"][name="' + obj1.attr('name') + '"]'),
						delimiter = obj1.data('delimiter') || ',',
						$obj2 = obj1.parents('.file-input'),
						$form_group = $obj2.parents('.form-group'),
						path = '';
					if (multiple) {
						fileinput_upindex++;
						...
						if (fileinput_upindex == fileinput_uplength) {
							...
							path = $obj1.val() ? $obj1.val() + delimiter + fileinput_upsrc : fileinput_upsrc;
							$obj1.val(path).change(); // input值更新
						}
					} else {
					...
					// 显示上传成功文字
					$obj2.find('.input-group .file-caption-name').html('<span class="glyphicon glyphicon-file kv-caption-icon"></span>' + path).attr({
						title: path
					});
					messageFun($obj2, 'success', METLANG.jsx17);
					// 更新上传文件预览地址
					var $preview_view = $obj2.find('.file-preview-thumbnails .file-preview-frame .file-preview-view[data-order="' + data.response.order + '"]:not([data-size])');
					$preview_view.attr({
						href: data.response.original,
						'data-size': data.response.filesize
					}).parents('.file-preview-frame').removeClass('hide');
					setTimeout(function() {
						$preview_view.parents('.file-preview-frame').addClass('file-preview-initial');
					}, 100);
				};
			$(this).each(function(index, el) {
				...
				$(this).removeAttr('hidden').fileinput({ // fileinput插件
					...
					window.fileinput_uplength = files.length;
					window.fileinput_upindex = 0;
					window.fileinput_upsrc = [];
					var $self = $(this),
						$is_compress = $(this).parents('.file-input').find('.file-preview-frame:not(.file-preview-initial) img').filter(function(index) {
							var result = this.naturalWidth > 2000 || this.naturalHeight > 2000;
							return result;
						}),
					...
					if (compress_length) {
						...
						$is_compress.each(function(index, el) {
							...
							var canvas = document.createElement('canvas'),
								...
								size = imgScale(2000, 2000, this.naturalWidth, this.naturalHeight),
								order = parseInt($(this).parent().find('.file-preview-view').attr('data-order'));
				...
				if (!$(this).parents('form').find('input[type="text"][name="' + name + '"]').length) $(this).after('<input type="text" name="' + name + '" value="' + value + '" ' + required + '>');
				// 验证初始化
				required && metui.use('app_form', function() {
					$self.parent().metFormAddField();
				});
				// 多图拖曳改变位置
				$(this).attr('multiple') && metui.use('dragsort', function() {
					...
					setTimeout(function() {
						dragsortFun[$thumbnails.attr('data-dragsort_order')] = function(wrapper, item) {
							...
							$self.parents('form').find('input[type="text"][name="' + name + '"]').val(img_url).change();
						}
					}, 100)
				});
				// 尺寸信息初始化
		...
		metFileInputChange: function(img_url) {
			...
			if ($(this).attr('multiple')) {
				var old_val = $file_input.find('input[type="text"][name="' + name + '"]').val();
				if (old_val) {
			...
			$file_input.find('.input-group .file-caption-name').html('<span class="glyphicon glyphicon-file kv-caption-icon"></span>' + img_url).attr({
				title: img_url
			});
			$file_input.find('input[type="text"][name="' + name + '"]').val(img_url).change();
		},
		// 更新图片尺寸
		imgSize: function() {
			var $self = $(this),
	...
	function fileThumbnail(imgurl) {
		var file_html = imgurl.indexOf('.png') >= 0 || imgurl.indexOf('.jpeg') >= 0 || imgurl.indexOf('.jpg') >= 0 || imgurl.indexOf('.bmp') >= 0 || imgurl.indexOf('.gif') >= 0 || imgurl.indexOf('.ico') >= 0 ? '<img src="' + imgurl + '" class="file-preview-image vertical-align-middle"></a>' : '<div class="file-preview-other text-grey" style="width:68px;"><i class="glyphicon glyphicon-file"></i></div>',
			html = '<a href="' + imgurl + '" target="_blank" class="d-block">' + file_html + '</a>' + '<div class="file-thumbnail-footer">' + '<div class="file-caption-name" title="' + imgurl + '">' + imgurl + '</div>' + '<div class="file-actions">' + '<a href="' + imgurl + '" class="btn btn-xs btn-default file-preview-view" title="'+METLANG.clickview+'" target="_blank"><i class="fa-eye text-grey"></i></a>' + '<div class="file-footer-buttons">' + '<button type="button" class="kv-file-remove btn btn-xs btn-default" title="' + $.fn.fileinputLocales[M.fileinput_lang].removeTitle + '"><i class="glyphicon glyphicon-trash text-grey"></i></button>' + '</div>' + '<div class="clearfix"></div>' + '</div>' + '</div>' + '</div>';
		return html;
	}
	// 上传图片尺寸缩放
	...
	$(document).on('click', '.file-input .file-preview-thumbnails .file-preview-frame .kv-file-remove,.fileinput-remove', function(event) {
		...
		var $file_input = $(this).parents('.file-input'),
			...
			$input_name = $file_input.find('input[type="text"][name="' + name + '"]'),
		...
		if (input_value) {
			if ($(this).hasClass('kv-file-remove')) {
				...
				setTimeout(function() {
					...
					$caption_name.html('<span class="glyphicon glyphicon-file kv-caption-icon"></span>' + path).attr({
						title: path
					});
				}, 1000)
	...
	// 文件地址更新后，图片尺寸更新
	$(document).on('change', '.file-input .btn-file input[type="text"][name]', function(event) {
		var $file = $(this).parent().find('input[type="file"]');
		setTimeout(function() {
			$file.data('size') && $file.imgSize();
			// 自定义回调
			var callback = $file.data('callback');
			callback && eval(callback + '($file)');
		}, 0);
	});
	var filelist_url = M.url.admin + '?n=system&c=filept&a=doGetFileList';
	// 图片库-点击按钮
	$(document).on('click', '.file-input .fileinput-file-choose', function(event) {
		...
		setTimeout(function() {
			$('.img-library-modal button[data-ok]').attr({
				'data-id': id
			});
		}, 0)
	});
	...
	$(document).on('show.bs.modal', '.img-library-modal', function(event) {
		var $loader = $('.img-library-loader', this),
			$img_folder_list = $('.img-folder-list', this);
		$('.modal-body>ul,.nodata', this).addClass('hide');
		$('.modal-body>ul', this).html('');
		...
		metui.ajax({
			...
			if (result) {
				...
				$.each(result, function(index, val) {
					html += '<li class="text-center my-4 col-6 col-md-4 col-lg-3 col-xl-2">' + '<a href="javascript:;" title="' + val.name + '" data-path="' + val.path + '" class="d-block text-content">' + '<i class="fa-folder-open-o h1"></i>' + '<h4 class="h6 mb-0">' + val.name + '</h4>' + '</a>' + '</li>';
				});
				$loader.addClass('hide');
				$img_folder_list.removeClass('hide').html(html);
			}
		});
	});
	// 图片库-选择文件夹
	$(document).on('dblclick', '.img-library-modal .img-folder-list li a,.img-library-modal .img-library-list li a.dir,.img-library-modal .btn-img-folder-back,.img-library-modal .img-library-breadcrumb a', function(event) {
		var $loader = $('.img-library-modal .img-library-loader'),
			$img_library_list = $('.img-library-modal .img-library-list'),
			dir=$(this).attr('data-path');
		if(!dir) return;
		$('.img-library-modal .modal-body>ul,.img-library-modal .nodata').addClass('hide');
		$('.img-library-modal .modal-body>ul').html('');
		$loader.removeClass('hide');
		metui.ajax({
			url: filelist_url,
			data: {
			...
			if (result) {
				var html = '';
				$.each(result, function(index, val) {
					if(val.type=='dir'){// 为文件夹的情况
						html += '<li class="text-center mb-2 col-6 col-md-4 col-lg-3 col-xl-2">'
								+ '<a href="javascript:;" title="' + val.name + '" data-path="' + val.path + '" class="d-flex align-items-center justify-content-center text-content vertical-align dir">'
									+'<div>'
						...
						html += '<li class="text-center mb-2 col-6 col-md-4 col-lg-3 col-xl-2 px-1">' + '<a href="javascript:;" title="' + val.name + '" class="d-flex align-items-center justify-content-center p-1 border position-relative img">' + '<img ' + (index > 5 ? 'data-original' : 'src') + '="' + val.value + '" class="vertical-align-middle img-fluid"/>' + '<i class="fa-check text-white position-absolute hide"></i>' + '</a>' + '</li>';
					}
				});
				$loader.addClass('hide');
				if (html){
					...
					$('.img-library-modal .nodata').removeClass('hide');
				}
				// 图片路径面包屑
				var breadcrumb=[],
					breadcrumb_path='';
				...
				$('.img-library-modal .img-library-breadcrumb').removeClass('hide').html(breadcrumb);
			}
		});
	});
	// 图片库-返回文件夹列表
	$(document).on('click', '.img-library-modal .btn-img-folder-back,.img-library-modal .img-library-breadcrumb a', function(event) {
		var $breadcrumb_a=$('.img-library-modal .img-library-breadcrumb a'),
	...
	// 图片库-选择图片
	$(document).on('click', '.img-library-modal .img-library-list li a.img', function(event) {
		var multiple = $('.file-input #' + $('.img-library-modal button[data-ok]').attr('data-id')).attr('multiple') ? true : false;
		$(this).toggleClass('active border-primary').find('i').removeClass('hide');
		if (!multiple) $(this).parents('li').siblings('li').find('a').removeClass('active border-primary').find('i').addClass('hide');
	});
	// 图片库-提交
	$(document).on('click', '.img-library-modal button[data-ok]', function(event) {
		...
		metui.use('alertify', function() {
			if (img_url) {
				...
				$file.parents('.form-group').find('.kv-upload-progress').addClass('hide');
			} else {
