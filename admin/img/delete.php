<?php
# 文件名称:delete.php 2009-08-11 19:09:57
# MetInfo企业网站管理系统 
...
$k=count($allidlist)-1;
for($i=0;$i<$k; $i++){
if($met_htmpagename==1)$news_list=$db->get_one("select * from $met_img where id='$allidlist[$i]'");
$updatetime=date('Ymd',strtotime($news_list[updatetime]));
deletepage($folder[foldername],$allidlist[$i],'showimg',$updatetime);
...
deletepage($folder[foldername],$id,'showimg',$updatetime);
indexhtm();
