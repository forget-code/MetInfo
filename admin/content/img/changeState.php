...
if($action=='copy'){
	...
	foreach($original as $key=>$val){
		...
		$query = "update {$met_img} SET class1='$moveclass1',class2='$moveclass2',class3='$moveclass3',access='$access',{$filname}lang='$movelang' where id='$val[id]'";
		$db->query($query);
		$query="select * from {$met_plist} where listid='{$val[id]}'";
		$plist=$db->get_all($query);
		foreach($plist as $key2=>$val2){
