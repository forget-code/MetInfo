...
if(!($db->get_one("SELECT * FROM $met_column WHERE id='$class1'"))){
	 $class1ins= $db->get_one("SELECT * FROM $met_column WHERE module='100' and lang='$lang'");
	 $class1=$class1ins[id];
	 if($search=='search'){
	 $productclassnum=$db->counter($met_column, " where module='3' and bigclass='0' and lang='$lang'", "*");
	 if($productclassnum==1) {
	    $productclassnumone=$db->get_one("SELECT * FROM $met_column WHERE module='3' and bigclass='0' and lang='$lang' ");
   ...
   if($title<>''){
	...
	$paratitle=$$val[para];
	 ...
	 $paratitle="";
	  ...
	  if(trim($paratitle)<>''){
	   $serch_sql .= " and exists(select * from $met_plist where module=3 and $met_plist.listid=$met_product.id and $met_plist.info='".trim($paratitle)."') "; 
	...
	if(trim($paratitle)<>''){
	   $serch_sql .= " and exists(select * from $met_plist where module=3 and $met_plist.listid=$met_product.id and $met_plist.info like'%".trim($paratitle)."%') ";  
	   $serchpage .= "&".$val[para]."=".trim($paratitle);
	   }
	 }
} 
	...
	$page = $page?$page:1;
	 $query = "SELECT $listitem[product] FROM $met_product $serch_sql and top_ok='1' $order_sql LIMIT $from_record, $list_num";
    ...
    if($dataoptimize[3][para][3]){
	  $query1 = "select * from $met_plist where listid='$list[id]' and module='3' ";
...
if($search=='search'){		
$page_list = $rowset->link("product.php?lang=$lang&class1=$class1&class2=$class2&class3=$class3".$serchpage."&search=search&page=");	
}else{
if($met_webhtm==2){
if($class3<>0){
...
$page_list = $rowset->link("product.php?lang=$lang&class1=$class1&class2=$class2&class3=$class3&page=");	
}
}
